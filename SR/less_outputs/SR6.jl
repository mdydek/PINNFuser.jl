...
function NIK_PINN!(du, u, p, t)
    ...
    du[1] = (Qmv - Qav) * ShiElastance(t, Eₘᵢₙ, Eₘₐₓ, τ, τₑₛ, τₑₚ, Eshift) +
        pLV / ShiElastance(t, Eₘᵢₙ, Eₘₐₓ, τ, τₑₛ, τₑₚ, Eshift) * 1
...
w1 = Optimization.solve(optprob, ADAM(0.01), callback=callback, maxiters = 1)
...
optprob1 = Optimization.OptimizationProblem(optf, w1.u)
PINN_sol = Optimization.solve(optprob1, ADAM(0.0001), callback=callback, maxiters = 1)
...
writedlm("data/pinn_data6.txt", data_to_save)
println("Data from training range saved to pinn_data6.txt")
...
writedlm("data/pinn_extrapolation6.txt", data_to_save)
println("Pinn extrapolation saved to pinn_extrapolation6.txt")
