...
s = solve(prob_NN, Vern7(), dtmax=1e-2, saveat = tsteps, reltol=1e-7, abstol=1e-6)

function predict(p)
    temp_prob = remake(prob_NN, p=p)
    temp_sol = solve(temp_prob, Vern7(), dtmax=1e-2, saveat = tsteps, reltol=1e-7, abstol=1e-6)
    return temp_sol
end
...
writedlm("data/pinn_data1.txt", data_to_save)
...
s = solve(trained_NN, Vern7(), dtmax=1e-2, saveat = tsteps, reltol=1e-7, abstol=1e-6)
...
writedlm("data/pinn_extrapolation1.txt", data_to_save)
