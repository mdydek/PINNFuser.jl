...
function NIK_PINN!(du, u, p, t)
    ...
    ωs = 2π .* (1:3)  # Use a few frequency bands
    time_features = time_embedding([t], ωs)  # a vector of length 8
    nn_input = vcat(u[[1, 5, 6]], time_features)  # shape: (3 + 8,)
...
function loss(p)
    ...
    # return data_loss + λ * periodic_loss(pred)
...
callback = function (p, l)
    ...
    if length(losses1_0)%1 == 0
        ...
        # grads = Zygote.gradient(() -> loss(p), p)
        # norms = map(x -> norm(x[2]), pairs(grads[1]))
        # println("Gradient norms: ", norms)
      end
    return false
  end
...
w1 = Optimization.solve(optprob, ADAM(0.01), callback=callback, maxiters = 120)
...
PINN_sol = Optimization.solve(optprob2, ADAM(0.0001), callback=callback, maxiters = 30)
...
writedlm("data/pinn_dataSINf.txt", data_to_save)
...
writedlm("data/pinn_extrapolationSINf.txt", data_to_save)
