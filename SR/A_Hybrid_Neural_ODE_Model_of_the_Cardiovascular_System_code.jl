...
function CVS!(du, u, p, t)
    ...
    Elvf, Eao, Evc, Ervf, Epa, Epu, Rmt, Rav, Rsys, Rtc, Rpv, Rpul, Lmt, Lav, Ltc, Lpv, Vdlvf, Vdao, Vdvc, Vdrvf, Vdpa, Vdpu, P0lvf, P0rvf,
    lambdalvf, lambdarvf, Espt, V0lvf, V0rvf, P0spt, P0pcd, V0spt, V0pcd, lambdaspt, lambdapcd, Vdspt, Pth = p
    ...
    while abs(dy) > 1e-5*abs(Vspt)
        f = e*Espt*(Vspt-Vdspt) + (1-e)*P0spt*(exp(lambdaspt*(Vspt-V0spt))-1) - e*Elvf*(Vlv-Vspt-Vdlvf) - (1-e)*P0lvf*(exp(lambdalvf*(Vlv-Vspt-V0lvf))-1) + e*Ervf*(Vrv+Vspt-Vdrvf) + (1-e)*P0rvf*(exp(lambdarvf*(Vrv+Vspt-V0rvf))-1)  

        df = e*Espt + lambdaspt*(1-e)*P0spt*exp(lambdaspt*(Vspt-V0spt)) + e*Elvf + lambdalvf*(1-e)*P0lvf*exp(lambdalvf*(Vlv-Vspt-V0lvf)) + e*Ervf + lambdarvf*(1-e)*P0rvf*exp(lambdarvf*(Vrv+Vspt-V0rvf))  
...
# Parameters
p_ = [2.8798, 0.6913, 0.0059, 0.585, 0.369, 0.0073, 0.0158, 0.018, 1.0889, 0.0237, 0.0055, 0.1552, 7.6968e-5, 1.2189e-4, 8.0093e-5, 1.4868e-4,
      0, 0, 0, 0, 0, 0, 0.1203, 0.2157, 0.033, 0.023, 48.754, 0, 0, 1.1101, 0.5003, 2, 200, 0.435, 0.03, 2, -4]

# Solve to generate synthetic "ground truth" data
prob = ODEProblem(CVS!, u0, tspan, p_)
@time solution = solve(prob, Tsit5(), dtmax=1e-2, abstol=1e-7, reltol=1e-4, saveat = tsteps)
...
Elvf, Eao, Evc, Ervf, Epa, Epu, Rmt, Rav, Rsys, Rtc, Rpv, Rpul, Lmt, Lav, Ltc, Lpv, Vdlvf, Vdao, Vdvc, Vdrvf, Vdpa, Vdpu, P0lvf, P0rvf,
lambdalvf, lambdarvf, Espt, V0lvf, V0rvf, P0spt, P0pcd, V0spt, V0pcd, lambdaspt, lambdapcd, Vdspt, Pth = p_
...
for i = 1:length(tsteps)
    while abs(dx[i]) > 1e-5*abs(Vspt[i])
        f = e[i].*Espt.*(Vspt[i].-Vdspt) .+ (1 .-e[i]).*P0spt.*(exp.(lambdaspt.*(Vspt[i].-V0spt)).-1) .- e[i].*Elvf.*(Vlv[i].-Vspt[i].-Vdlvf) .- (1 .-e[i]).*P0lvf.*(exp.(lambdalvf.*(Vlv[i].-Vspt[i].-V0lvf)).-1) .+ e[i].*Ervf.*(Vrv[i].+Vspt[i].-Vdrvf) .+ (1 .-e[i]).*P0rvf.*(exp.(lambdarvf.*(Vrv[i].+Vspt[i].-V0rvf)).-1)
        
        df = e[i].*Espt .+ lambdaspt.*(1 .-e[i]).*P0spt.*exp.(lambdaspt.*(Vspt[i].-V0spt)) .+ e[i].*Elvf .+ lambdalvf.*(1 .-e[i]).*P0lvf.*exp.(lambdalvf.*(Vlv[i].-Vspt[i].-V0lvf)) .+ e[i].*Ervf .+ lambdarvf.*(1 .-e[i]).*P0rvf.*exp.(lambdarvf.*(Vrv[i].+Vspt[i].-V0rvf))
...
function predict(p)
    ...
    temp_sol = solve(temp_prob, Vern7(), dtmax=1e-2, abstol=1e-7, reltol=1e-4, saveat=solution.t)
    return temp_sol
end
...
avgIn_0 = (prediction1_0 + prediction2_0 + prediction3_0 + prediction4_0 + prediction5_0
          + prediction6_0 + prediction7_0 + prediction8_0 + prediction9_0 + prediction10_0)/10

# Take mean of learned network dynamics in the ensemble (target for SR)
avgNN_0 = (NN_dynamics1_0  + NN_dynamics2_0 + NN_dynamics3_0 + NN_dynamics4_0 + NN_dynamics5_0
          + NN_dynamics6_0 + NN_dynamics7_0  + NN_dynamics8_0 + NN_dynamics9_0  + NN_dynamics10_0)/10
...
learned_Vspt0 = (970.5831 ./ (Vao .- Vpa)) .- (7.331516 .- (12.834902 ./ (Vao .+ (2.5382192 .- Vrv))))

# Learned terms with 2% noise
learned_Pperi2 = ((Vrv .+ -91.48883) ./ Vpa) .+ -4.1087065
learned_Vspt2 = -8.62723 .- ((-1080.363 .- exp.(Vrv ./ (Vpa .+ -10.053128))) ./ (Vao .- Vpa))

# Learned terms with 5% noise
learned_Pperi5 = ((Vrv .+ -88.12736) ./ Vpa) .+ -4.2060776
learned_Vspt5 = ((Vvc ./ (Vao .- Vpa)) .+ -2.4558141) .* exp.(exp.(Vrv ./ (Vpa .* (Vpa .* 0.29237527))))

# Visualise (averaged) learned dynamics of the models as well as the dynamics of the learned terms
fig6 = plot!(tsteps, Pperi, xlabel="time (s)", ylabel="pressure (mmHg)", label="Pperi",
legend=:bottom, legendfontsize=9, color=:black, linewidth=2, guidefontsize=:13, size=(650,350))
...
plot!(tsteps, NN_dynamics6_0[1,:], label="Model 6", linewidth=1.5, color=:mediumvioletred)
plot!(tsteps, NN_dynamics7_0[1,:], label="Model 7", linewidth=1.5, color=:chocolate)
plot!(tsteps, NN_dynamics8_0[1,:], label="Model 8", linewidth=1.5, color=:navajowhite4)
plot!(tsteps, NN_dynamics9_0[1,:], label="Model 9", linewidth=1.5, color=:darkgoldenrod2)
...
solution2 = solve(prob2, Vern7(), dtmax=1e-2, abstol=1e-7, reltol=1e-4, saveat = tsteps2)
...
@time train_sol = solve(train_prob, Tsit5(), dtmax=1e-2, abstol=1e-7, reltol=1e-4, saveat=solution2.t)
...
function Learned!(du, u, p, t)
    ...
    Vspt = (970.5831 ./ (Vao .- Vpa)) .- (7.331516 .- (12.834902 ./ (Vao .+ (2.5382192 .- Vrv))))
...
@time SRsol = solve(SRprob, Tsit5(), dtmax=1e-2, abstol=1e-7, reltol=1e-4, saveat = tsteps2)
SR_ex = Array(SRsol)
...
@time NoVIsol = solve(NoVIprob, Vern7(), dtmax=1e-2, abstol=1e-7, reltol=1e-4, saveat = tsteps2)
