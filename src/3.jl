...
# function loss(p::AbstractVector)
#     pred = predict(p)
#     pred_array = Array(pred)

#     println("PRED: ", pred_array)
#     println("ORIGINAL DATA: ", original_data)

#     pred_vals = ForwardDiff.value.(pred_array)  # <-- tu magia ✨

#     println("PRED CONVERTED: ", pred_vals)
...
function loss(p)
    ...
    # println(Array(pred))
    # println(pred[1])
    # println(size(pred), size(Array(pred)), size(pred[1, :]), size(Array[pred[1, :]]))

    # Wybieramy zmienne z predykcji, zakładając że pred.u to macierz 300x2
    # Stosujemy ForwardDiff.value do uzyskania wartości (nie pochodnych)
    pred_vals = hcat(
...
writedlm("pinn_data.txt", data_to_save)
