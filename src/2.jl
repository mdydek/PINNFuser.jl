...
function loss(p; alpha=0.1, beta=0.01)
    ...
    τₑₛ, τₑₚ, Rmv, Zao, Rs, Csa, Csv, Eₘₐₓ, Eₘᵢₙ = params
    phys_loss1 = mean(abs2, dpred_dt[:, 5] .- (pred_vals[2:end, 1] .- pred_vals[2:end, 2]) ./ Zao)
    phys_loss2 = mean(abs2, dpred_dt[:, 6] .- (pred_vals[2:end, 3] .- pred_vals[2:end, 1]) ./ Rmv)
    phys_loss3 = mean(abs2, dpred_dt[:, 7] .- (pred_vals[2:end, 2] .- pred_vals[2:end, 3]) ./ Rs)
