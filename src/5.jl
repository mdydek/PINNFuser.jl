...
# ===================== Fourier features ===================== #
period = 1.0       # approx cardiac cycle (seconds)
...
# ===================== ODE system ===================== #
function NIK_PINN!(du, u, p_nn, t)
    pLV, psa, psv, Vlv, Qav, Qmv, Qs = u
    τₑₛ, τₑₚ, Rmv, Zao, Rs, Csa, Csv, Eₘₐₓ, Eₘᵢₙ = params

    # Input = state + Fourier features
    tfeat = fourier_features(t, K, period)
    NN_input = vcat(u, tfeat)
    NN_output = NN(NN_input, p_nn, st)[1]
...
function physics_loss(pred, p_nn)
    loss = 0.0
    for (i, t) in enumerate(tsteps)
        u = pred[:, i]
        du = similar(u)
        NIK_PINN!(du, u, p_nn, t)
        τₑₛ, τₑₚ, Rmv, Zao, Rs, Csa, Csv, Eₘₐₓ, Eₘᵢₙ = params
