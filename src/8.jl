...
function NIK_PINN!(du, u, p, t, α)
    ...
    # Obliczamy korekcję NN
    NN_output = NN(u, p, st)[1]
    
    # Skalujemy po elementach przez du_phys (proporcjonalna korekcja)
    for i in 1:7
...
# === GRID SEARCH PO α W WIELOWĄTKOWOŚCI ===
alphas = [0.1, 0.3, 0.8]
...
@threads for i in 1:length(alphas)
    ...
    w = Optimization.solve(optprob, ADAM(0.01), callback=cb, maxiters=250)
...
writedlm("src/data/5_long_training_correction_extrapolation.txt", data_to_save)
