...
α = 0.3
lambdas = [20, 40, 50]
results = Vector{Tuple{Float64, Float64, ComponentVector{Float64}}}(undef, length(lambdas))

@threads for i in 1:length(lambdas)
    # α = alphas[i]
    λ = lambdas[i]
    println("==> Start treningu dla lambda = $λ (wątek $(threadid()))")
    
    cb = make_callback(α, print_every=3)
    adtype = Optimization.AutoForwardDiff()
    optf = Optimization.OptimizationFunction((x,p) -> total_loss(x, α, λ), adtype)
    ...
    final_loss = total_loss(final_sol.u, α, λ)
    results[i] = (λ, final_loss, final_sol.u)
    
    println("==> α = $λ zakończony, loss = $final_loss")
end

best_idx = argmin([r[2] for r in results])
best_lambda, best_loss, best_params = results[best_idx]
println("\nNajlepszy α = $best_lambda z loss = $best_loss")

PINN_sol_u = best_params

tspan2 = (0.0, 60.0)
tsteps2 = range(0.0, 60.0, length=9000)
trained_NN = ODEProblem((du,u,p,t)->NIK_PINN!(du,u,p,t,α), u0, tspan2, PINN_sol_u)
s = solve(trained_NN, Vern7(), dtmax=1e-2, saveat=tsteps2, reltol=1e-7, abstol=1e-4)
data_to_save = hcat(s[1, :], s[2, :], s[3, :], s[4, :], s[5, :], s[6, :], s[7, :])
writedlm("src/data/5b_correction_extrapolation.txt", data_to_save)

println("Zapisano dane dla najlepszej λ = $best_lambda")
